...
export function activate(context: vscode.ExtensionContext) {
	...
	async function writeSerializedData(serializedData: string, filename: string = 'comments.json') {
		const fileHandler = new FileHandler();
		try {
	...
	function serializeCommentThread(thread: vscode.CommentThread) {
		return {
			...
			comments: thread.comments.map(comment => (comment as NoteComment).toJSON()),
			// collapsibleState: thread.collapsibleState
			collapsibleState: vscode.CommentThreadCollapsibleState.Collapsed
		};
	}

	function serializeCommentThreads(threads: vscode.CommentThread[]): string {
	...
	async function restoreCommentsFromJSON(jsonData: string, commentController: vscode.CommentController) {
		...
		for (const threadData of threadsData) {
			...
			const range = new vscode.Range(
				new vscode.Position(threadData.range.start.line, threadData.range.start.character),
				new vscode.Position(threadData.range.end.line, threadData.range.end.character)
			);
